...
object Problem5{
  ...
  def main (args: Array[String]) {
    ...
    val jointWeights = PrecompiledTrainers.trainPerceptron(jointTrain,jointModel.feat,jointModel.predict,12)
...
case class JointConstrainedClassifier(triggerLabels:Set[Label],
  ...
  def predict(x: Candidate, weights: Weights) :(String, Seq[String]) ={
    ...
    def func3(inputString: String ):String = {
      if (inputString.equals("Cause")){
        //println("Made it")
        //如果遇到非Regulation家族的但是却带有Cause的 就把对应的那个Cause换成 Theme
        return "None"
      }
      return inputString
    }
    ...
    var bestArguments = Seq[Label]("")
    if (bestTrigger.toString().equals("None")){ //Constraint 1
      bestArguments = for (arg<-x.arguments) yield "None"

      return (bestTrigger,bestArguments)
    }
    else if(!(bestTrigger.equals("Regulation") || bestTrigger.equals("Positive_regulation") || bestTrigger.equals("Negative_regulation"))) {
      //Constraint 3
      bestArguments = for (arg <- x.arguments) yield func3(argmax(argumentLabels, arg, weights, argumentFeature))
      //println(bestTrigger,bestArguments)
      var TempArgument = Seq[Label]("")
      if(!bestArguments.contains("Theme")){
        if(bestArguments.isDefinedAt(3))
          TempArgument =  bestArguments.updated(3 , "Theme")
        else if(bestArguments.isDefinedAt(2))
          TempArgument =  bestArguments.updated(2 , "Theme")
        else if(bestArguments.isDefinedAt(1))
          TempArgument =  bestArguments.updated(1 , "Theme")
        else
          TempArgument =  bestArguments.updated(0 , "Theme")
      }
      else
        TempArgument = bestArguments
      return (bestTrigger,TempArgument)
    }
    else{
