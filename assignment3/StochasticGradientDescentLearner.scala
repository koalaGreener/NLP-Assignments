...
object StochasticGradientDescentLearner extends App {
  def apply(model: Model, corpus: String, maxEpochs: Int = 20, learningRate: Double, epochHook: (Int, Double) => Unit): Unit = {
    //number of tweets
    ...
    for (i <- 0 until maxEpochs) {
      ...
      for (j <- 0 until iterations) {
        ...
        val tempBlock=model.loss(sentence, target)
        tempBlock.backward()
        tempBlock.update(learningRate)
        accLoss += tempBlock.forward()
      }
      epochHook(i, accLoss)
    }
  }
